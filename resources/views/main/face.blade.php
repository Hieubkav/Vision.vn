...
@section('content')
    ...
    <div class="body_video flex justify-center items-center">
        <video src="" id="videoElm" autoplay='true' class=""></video>
    </div>
        ...
        video.addEventListener('playing', () => {
            ...
            setInterval(async () => {
                ...
                if (detection) {
                    ...
                    const eyeWidth = (euclideanDistance(leftEye[0], leftEye[3]) + euclideanDistance(
                    ...
                    const faceShape = determineFaceShape(jawWidth, faceHeight, noseHeight, eyeDistance,
                    ...
                    new faceapi.draw.DrawTextField(
                        [
                            ...
                            `=> Bạn phù hợp với kính: ${kinh_hop(resizedDetection.age, resizedDetection.gender)}` ,
                        ],
                    ...
                    function kinh_hop(age,gender){
                        let tu_van = "Không có ";
                        if (gender === 'male') tu_van = "Kính đen ,có ve kính";
                        else if (gender === 'female') tu_van = "Mang kính gì cũng đẹp ^^";
...
@section('content')
    ...
    <script>
        ...
        function make_link(inputString) {
            link = "http://127.0.0.1:8000/product/filter"
        ...
        async function kinh_hop_api(age, gender, faceShape) {
            const prompt =
                `Given that a person is ${age} years old, identifies as ${gender}, and has a ${faceShape} face shape, what type of glasses would be most suitable?`;

            const response = await fetch('https://api.openai.com/v1/engines/davinci-codex/completions', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer 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`,
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    prompt,
